...
// TestComposeServerReportsTheSameExtractorTheWorkerRuns pins the pair that makes /healthz's `extraction`
// field mean anything.
//
// Extraction runs in the worker; the field is reported by serve, from serve's own configuration. That is a
// useful answer only while both roles read the same two variables — otherwise the endpoint confidently names
// a model that is not the one distilling anything, which is worse than the empty answer it replaced. So this
// asserts the server carries them AND that both services resolve them from the same host variable.
//
// It also pins an absence: ANTHROPIC_API_KEY must NOT reach the server. Naming the extractor takes only the
// provider and model, and handing a process a credential it has no code path for widens the blast radius of
// a leak for nothing. The temptation is real — the obvious way to report an identity is to build the thing
// and ask it — so the constraint is written down rather than left to memory.
func TestComposeServerReportsTheSameExtractorTheWorkerRuns(t *testing.T) {
	rendered, err := renderCompose("v0.0.0")
	if err != nil {
		t.Fatalf("renderCompose: %v", err)
	}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			server := serviceEnv(t, tc.data, "lore-server")
			worker := serviceEnv(t, tc.data, "lore-worker")

			for _, key := range []string{"LORE_EXTRACTION_PROVIDER", "LORE_EXTRACTION_MODEL"} {
				got, ok := server[key]
				if !ok {
					t.Errorf("lore-server is missing %s — /healthz would report the fixture extractor "+
						"however the worker is configured", key)
					continue
				}
				if got != worker[key] {
					t.Errorf("%s differs — server=%q worker=%q; /healthz would name an extractor that is "+
						"not the one distilling", key, got, worker[key])
				}
			}

			if _, leaked := server["ANTHROPIC_API_KEY"]; leaked {
				t.Error("lore-server carries ANTHROPIC_API_KEY; serve builds no extractor, so the key has " +
					"no code path there — naming the identity needs the provider and model only")
			}
		})
	}
}
