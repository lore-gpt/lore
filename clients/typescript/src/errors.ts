// The errors the SDK throws. Everything is a LoreError; API failures are a discriminated union keyed by the
// server's machine `code`, so a caller switches on `code` and never string-matches a message.
import type { WireError } from "./wire.ts";

/** The machine error codes the SDK models explicitly. Unmodelled or absent codes surface as {@link UnknownLoreError}. */
...
/** Base class for everything the SDK throws. Discriminate on `code`. */
...
/**
 * A server error whose `code` the SDK does not model — a new code, or an absent one (the API's error schema
 * makes `code` optional). Keyed by HTTP status; the raw code, if any, is on `rawCode`.
 */
 ...
 * unmodelled code becomes an {@link UnknownLoreError} keyed by status — the SDK never guesses a code.
 */
