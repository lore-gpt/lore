...
// hand-written REST client and src/generated/openapi.ts: a spec rename or removal breaks the typecheck HERE (a
// second drift signal beyond the git-diff gate), not scattered across the client. Kept byte-for-byte parallel
// to the TypeScript SDK's wire.ts so the two clients cannot diverge from the same spec.
